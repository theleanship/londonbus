	...
	$.getJSON("http://127.0.0.1:2001/route/" + number, function(data) {
		...
		for(route in data) {
			...
			for(run in data[route]['runs']) {
				...
				data[route]['runs'][run].points = decodedPath;
				data[route]['runs'][run].distances = distances;

			}
			updateRoute(data[route].route, data[route]['runs']);
		}
